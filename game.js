...
export default class GameScene extends Phaser.Scene {
    ...
    getBiome(x, y) {
        ...
        // Right (X > 6000): Desert
        // Top (Y < 2000): Snow
        // Bottom (Y > 6000): Forest
        // Center: Meadow

        let biome = 'meadow';

        if (tileX < 2000) biome = 'lava';
        else if (tileX > 6000) biome = 'sand';
        else if (tileY < 2000) biome = 'snow';
        else if (tileY > 6000) biome = 'forest';

        // Water Logic (Scatter everywhere EXCEPT Lava)
        if (biome !== 'lava') {
            const scale = 0.001;
    ...
    updateAtmosphere(delta) {
        ...
        const isForest = this.player.y > 6000 && this.player.x >= 2000 && this.player.x <= 6000;
        if (isForest) {
            const fogAlpha = Math.min(0.5, (this.player.y - 6000) / 1500);
    ...
    createBiomes() {
        ...
        for (let y = 0; y < rows; y++) {
            for (let x = 0; x < cols; x++) {
                ...
                if (posX < 2000) texture = 'lava';
                else if (posX > 6000) texture = 'sand';
                else if (posY < 2000) texture = 'snow';
                else if (posY > 6000) texture = 'forest';
    ...
    createMinimap() {
        ...
        for (let y = 0; y < rows; y++) {
            for (let x = 0; x < cols; x++) {
                ...
                if (posX < 2000) color = 0x3E2723; // Lava
                else if (posX > 6000) color = 0xE6C288; // Desert
                else if (posY < 2000) color = 0xE0F7FA; // Snow
                else if (posY > 6000) color = 0x1B5E20; // Forest
